...
func UpdateProduct(ctx *gin.Context) {
	...
	conf := &restapi.UpdateConf{
		UpdateReq:    reqParams,
		ModelObj:     obj,
		OptModelFunc: func(db *gorm.DB) *response.ErrorInfo { return optProductModelForUpdate(reqParams, obj) },
	}

	restapi.Update(ctx, conf)
}

func optProductModelForUpdate(reqParams *UpdateProductReq, obj *model.Product) *response.ErrorInfo {
